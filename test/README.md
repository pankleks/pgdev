...
| `unit/*.test.mjs` | no | `node:test` suites for the pure logic: the statement splitter, the SQL formatter (function-call spacing including schema-qualified calls, clause keywords and dollar-body handling), the query-shape helpers (`canUseCursor`, `requiresAutocommit`, `parseMaxRows`), CSV escaping, the SQL reference helpers used by completions, tab reordering and session persistence markers, the table-editor diff (change-only ALTER emission and its validations), the row-editor gate and planner (plain-select detection including expression/alias rejections, the parameterized UPDATE builder and its validations), the cell-editor type mapping and temporal/JSON conversions, JSON pretty-printing, the results store's export drain (captured grid, non-retained streaming, consumed-cursor state), the tab-session serializer, storage failure reporting and session round-trips, and query cancellation over a Unix-domain socket. |
| `browser/app.mjs` | yes + Chrome | Opt-in (`PGDEV_BROWSER=1`). Drives real Chrome over the DevTools Protocol: connects through the dialog, opens DDL tabs and asserts the editability rules (materialized view read-only, table/view/function editable), asks the completion provider for suggestions and asserts there are no exact duplicates, that overloads and same-named functions in other schemas are listed separately, that a function sharing a column name stays visible, that built-ins appear once typed (and never unfiltered or `pg_`-prefixed), plus the user function, view, materialized view and stored procedure suggestions and schema-qualified suggestions, checks that collapse is inert while filtering, confirms a server error reaches the Messages tab, asserts the SQL token colors (`UPDATE` matches the other keywords, string literals are not pure red, function calls are distinct from column and table names), exercises the row editor (edit buttons only on PK-complete results, type-matched controls and locked primary key, a single-line array literal control, a rejected save staying open, a successful save patching the grid and reaching the database), and that Run is disabled once disconnected. |
