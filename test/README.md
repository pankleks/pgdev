...
| `unit/*.test.mjs` | no | `node:test` suites for the pure logic: the statement splitter, the query-shape helpers (`canUseCursor`, `requiresAutocommit`, `parseMaxRows`), CSV escaping, the SQL reference helpers used by completions, tab reordering, and query cancellation over a Unix-domain socket. |
