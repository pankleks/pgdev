...
- Partition bounds without duplicate `FOR VALUES` clauses; a default partition is emitted as bare `DEFAULT` rather than `FOR VALUES DEFAULT`.
...
Aggregate DDL emits `SFUNC`, `STYPE`, `FINALFUNC`, `COMBINEFUNC`, `SERIALFUNC`, `DESERIALFUNC`, `INITCOND`, `SORTOP` (wrapped as `OPERATOR(...)`) and `PARALLEL`. Range type DDL emits `SUBTYPE`, `SUBTYPE_OPCLASS`, `CANONICAL` and `SUBTYPE_DIFF`, omitting the last two unless the catalog actually defines them, since a `regproc`/`regclass` cast renders oid 0 as `-` rather than NULL.
...
Generated DDL was subsequently round-tripped against a live PostgreSQL 16.14 server: each object was created, its DDL generated, the object dropped, and the generated text re-executed verbatim and compared by catalog fingerprint (`pg_attribute`, `pg_get_constraintdef`, `pg_get_indexdef`, `pg_get_triggerdef`, `pg_get_viewdef`, `pg_range`, `pg_aggregate`). Every object in the 131-table `Pulsar_DEV` schema (1,538 objects) generated without error. That pass found and fixed three defects — the range-type `SUBTYPE_OPCLASS`/`CANONICAL`/`SUBTYPE_DIFF` values, the aggregate `SORTOP`, the duplicate per-partition foreign keys, and the `FOR VALUES DEFAULT` bound — and left the gaps listed below.

## TODO

### DDL generation gaps confirmed by live testing

- Emit `FINALFUNC_EXTRA` for aggregates whose `pg_aggregate.aggfinalextra` is set. Without it the final function is resolved with the wrong arity, so the generated DDL for `array_agg` and `percentile_disc` fails with `42883` and cannot be executed. This is the only gap here that makes a common built-in unusable.
- Emit `HYPOTHETICAL` for aggregates with `aggkind = 'h'`. The DDL applies but silently recreates them as ordered-set aggregates, which have different execution semantics.
- Emit the moving-aggregate and modification options that are currently dropped: `MSFUNC`, `MINVFUNC`, `MSTYPE`, `MFINALFUNC`, `MFINALFUNC_EXTRA`, `MFINALFUNC_MODIFY`, `SSPACE`, `MSSPACE`, `FINALFUNC_MODIFY`. Their absence loses the window-function implementation of `sum`/`avg` (correct results, worse performance) and changes `aggfinalmodify` from `'s'` back to the default `'w'`.
- Emit `COLLATION` and `MULTIRANGE_TYPE_NAME` in range type DDL. A range created with an explicit multirange name loses it (the default name is regenerated), so references to that name fail with `42704`; an explicit collation silently reverts to the default.
- Emit `PARTITION BY` for a partition that is itself partitioned (sub-partitioning). The child is currently recreated as a plain table (`relkind` `p` → `r`), after which the grandchild's `PARTITION OF` fails with `42P17`.
- Emit `COLLATE` for composite type attributes, which is currently dropped, so an attribute declared `COLLATE "C"` reverts to the default collation.
- Emit the constraint name for domain constraints. `pg_get_constraintdef` carries no name, so a domain constraint declared `CONSTRAINT pos` is recreated under the auto-generated name.
- Note that a range type with a `CANONICAL` function can never be recreated by a standalone script: PostgreSQL requires the shell type and the canonical function (which must be written in C) to exist first.
- Consider emitting `WITH NO DATA` for a materialized view created without data; the DDL currently applies but the recreated view is populated.
...
- Add PostgreSQL integration tests that execute generated DDL against tables, partitions (including default, list and hash bounds), RLS policies, identity columns, generated columns, serial columns, aggregates, range types, domains, and unusual identifiers. The round-trip harness described above is the model to automate.
