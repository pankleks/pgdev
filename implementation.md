...
The implementation is designed for local or trusted use. It does not provide user authentication. Database credentials are sent to the backend only when a connection is opened, and optional remembered connections are stored in the browser's IndexedDB.
...
- `server/src/catalog/ddl.ts` reconstructs executable DDL from PostgreSQL catalog data — tables, views, functions and aggregates, indexes, constraints, triggers, types, and sequences (`sequenceDdl` reuses the identity/serial option emitter, so START/INCREMENT/MIN/MAX/CACHE/CYCLE and `OWNED BY` round-trip).
...
- `server/src/ai/tools.ts` implements the agent tools (schema, DDL, read-only query, editor and tab actions) with row and byte caps, on the connection pgDEV has open.
...
- `web/src/components/EditorTabs.vue` manages tab display, closes associated backend sessions, and reorders tabs by drag and drop; double-clicking the empty tab strip opens a new query tab. Tabs the agent opened carry the `agent-tab` class, which tints their titles with `--agent` so staged SQL is recognizable in the strip.
...
- `web/src/lib/storage.ts` owns IndexedDB persistence for settings, connections, pinned files, and the tab session. When IndexedDB is unavailable every save rejects and the app runs on defaults; pinned-file saves warn through the UI.
...
The surface is always on. The server reads a stable per-user token from `~/.config/pgdev/token` (`server/src/ai/token.ts` mints and stores it on first run, with owner-only permissions; delete the file to rotate, and an unreadable location degrades to an ephemeral token with a warning); the **AI** dialog in the toolbar shows the server URL, the token, and a ready-to-paste MCP client configuration from `GET /api/ai/config`. `bin/pgdev-mcp.mjs` is a stdio MCP server built on the official `@modelcontextprotocol/sdk` that an MCP client starts; it publishes the tools and forwards each call to `POST /api/ai/tool/:name` with the token, which the server compares with `timingSafeEqual`. Tool calls are the one place where the drive-by-origin rule does not apply — an agent is not a browser and sends no `Origin` — so the token is the only credential. Anyone able to read the token file can drive the agent tools; pgDEV stays a local-machine application and the file lives outside the repository.
...
- `list_tabs`, `activate_tab`, `close_tab` — the agent's own tabs. `open_query_tab` marks what it creates (`agentOpened`, next to the `aiMirror` flag the result mirror already uses); `list_tabs` reports exactly those tabs with a dirty flag each, `activate_tab` switches to one by key or unique title, and `close_tab` closes one with the same cleanup as the UI (drop its result state, close its backend session). A tab the agent did not open is refused by name, and a dirty one — staged, then modified — is refused because only the user can confirm closing it.
...
The agent works on the connection pgDEV has open and cannot see or pick another one: there is no tool or resource that lists connections, `connection` is not an argument, and the database tools resolve the target as the connection named by the single listening window. An explicitly disconnected listening window is authoritative and fails with "pgDEV is not connected to a database" rather than falling back to a stale pool. With no window, a single pool remains the fallback; with several pools and no window to say which one is in use, the tools fail and say so too.

Each statement's rows are capped at 100 rows and 64 KB (`DEFAULT_AI_LIMITS` in `server/src/ai/tools.ts`), with the cap reported rather than silent. The first result of a `query` is also mirrored into an explicitly owned `AI` tab bound to the source connection — the agent's SQL and its rows — so the user can see what the agent read; a matching idle mirror is reused, while a tab with active work, an open transaction, a different connection, or anything merely titled `AI` (user query tabs, staged `open_query_tab` output titled `Agent SQL`, editable DDL previews) is preserved and another mirror tab is opened. The mirror render itself refuses busy tabs, so a raced `show-result` falls back to a fresh tab instead of detaching a running query from its cancellation UI. There is deliberately no chat UI in pgDEV, because prompts and answers belong to the agent's own client.
...
The same run covers the agent surface: a tool call without or with the wrong token is rejected, a read-only `query` returns its rows on the open connection without a browser window, a write is refused by the classifier, a data-modifying CTE — which classifies as a read — is refused by the read-only transaction, filtered schema and DDL reads, the explicit no-window error, a fresh server with nothing connected reporting that it is not connected, and the limits endpoint validating its range and applying a changed row limit to the next `query`. The browser suite then repeats it against a live page: the agent reads and replaces the active tab, stages a `CREATE VIEW` in a new tab that nothing runs, reads the schema of the connection the window has open, finds no tool that runs the editor or lists connections, sees its query result in the `AI` tab, reads the rows and messages of what the user ran (including a failed statement's error), honours a row limit changed in Settings, and manages its own tabs (lists, switches, closes — refused for dirty and foreign tabs).
...
- Note that remembered connections are stored unencrypted in the browser's IndexedDB. Replace that with an operating-system or external secret store where deployment requirements justify it.
