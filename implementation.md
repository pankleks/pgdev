...
Each statement's rows are capped at 100 rows and 64 KB (`DEFAULT_AI_LIMITS` in `server/src/ai/tools.ts`), with the cap reported rather than silent. The first result of a `query` is also mirrored into an `AI` tab bound to the source connection — the agent's SQL and its rows — so the user can see what the agent read; a matching idle tab is reused, while a tab with active work or a different connection is preserved and another tab is opened. There is deliberately no chat UI in pgDEV, because prompts and answers belong to the agent's own client.
